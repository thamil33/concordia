...
from agents.deprecated import deprecated_agent
...
from contrib.components.orchestrator import deprecated as gm_contrib
...
def _get_wage_from_orchestrator_memory(
...
class Simulation(scenarios_lib.RunnableSimulationWithMemories):
  ...
  def __init__(
    ...
    if antagonist_player is not None and organizer_player is not None:
      ...
      orchestrator_memory.add(called_for_a_strike)
    ...
    def inventory_effect_function(
      ...
      wage = _get_wage_from_orchestrator_memory(
    ...
    self._scenes, decision_env, industrial_action = configure_scenes(
        model=self._model,
        orchestrator_memory=self._orchestrator_memory,
        players=self._all_players,
        clock=self._clock,
