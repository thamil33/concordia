...
from agents.deprecated import entity_agent
...
from contrib.components.orchestrator.deprecated import agreement_tracker
from contrib.components.orchestrator.deprecated import daily_activities
from deprecated.factory.agent import basic_agent
from deprecated.factory.environment import basic_orchestrator
from document import interactive_document
from environment.deprecated import orchestrator
from environment.deprecated.scenes import conversation
...
from typing_custom.deprecated import entity as entity_lib
...
def configure_scenes(
    ...
    no_conversation_orchestrator: orchestrator.GameMaster,
    negotiation_orchestrator: orchestrator.GameMaster,
  ...
  Args:
    ...
    supporting_player_configs: configs for the supporting characters
    player_configs_dict: dict mapping player name to corresponding config
    no_conversation_orchestrator: secondary game master that does not include
      conversations
    negotiation_orchestrator: secondary game master for the negotiation scene
  ...
  scene_types[NEGOTIATION_SCENE_TYPE] = scene_lib.SceneTypeSpec(
      ...
      override_orchestrator=negotiation_orchestrator,
  )
  scene_types[POST_NEGOTIATION_SCENE_TYPE] = scene_lib.SceneTypeSpec(
      ...
      override_orchestrator=no_conversation_orchestrator,
  )
...
def get_daily_activities_component(
  ...
  def _are_agricultural_resources_pooled(
    ...
    retrieved = orchestrator_memory.retrieve_by_regex(
        regex=r'\[' + timepoint + r'\].*',
        sort_by_time=True,
    )
    result = False
  ...
  def player_score_fn(
    ...
    if _are_agricultural_resources_pooled(orchestrator_memory,
                                          timepoint):
...
class Simulation(scenarios_lib.RunnableSimulationWithMemories):
  ...
  def __init__(
    ...
    self._primary_environment, self._orchestrator_memory = (
    ...
    agreement_component = agreement_tracker.AgreementTracker(
        model=model,
        memory=self._orchestrator_memory,
    ...
    self._no_conversation_orchestrator = orchestrator.GameMaster(
        model=model,
        memory=self._orchestrator_memory,
        clock=self._clock,
    ...
    self._negotiation_orchestrator = conversation.make_conversation_orchestrator(
    ...
    self._scenes = configure_scenes(
        ...
        no_conversation_orchestrator=self._no_conversation_orchestrator,
        negotiation_orchestrator=self._negotiation_orchestrator,
    )
    self._secondary_environments = [self._negotiation_orchestrator,
                                    self._no_conversation_orchestrator]
  ...
  def _init_premise_memories(
    ...
    for premise in scenario_premise:
      self._orchestrator_memory.add(premise)
      for player in self._all_players:
        player.observe(premise)

    for shared_memory in shared_memories:
      self._orchestrator_memory.add(shared_memory)
      for player in self._all_players:
    ...
    for player_config in player_configs:
      ...
      for extra_memory in extra_memories:
        self._orchestrator_memory.add(extra_memory)
    ...
    for idx, player in enumerate(main_players):
      ...
      self._orchestrator_memory.add(scene_premise)
    ...
    for idx, player in enumerate(supporting_players):
      ...
      self._orchestrator_memory.add(teleport)
  ...
  def __call__(self)-> tuple[logging_lib.SimulationOutcome, str]:
    ...
    """
    html_results_log = basic_orchestrator.run_simulation(
        model=self._model,
        players=self._all_players,
