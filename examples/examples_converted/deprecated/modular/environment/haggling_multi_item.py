...
from contrib.components.orchestrator.deprecated import bargain_payoffs_multi_item as bargain_payoffs_lib
...
from typing_custom.deprecated import agent as agent_lib
...
def add_choice_scene_spec(
  ...
  Args:
    ...
    orchestrator_memory: the game master memory to use.
...
def configure_scenes(
    ...
    list[orchestrator.GameMaster] | list[None],
  ...
  for i in range(sampled_settings.num_games * len(pairs)):
    ...
    orchestrators.append(choice_scene_spec.override_orchestrator)
  ...
  # choice_scene_spec.override_orchestrator
  return (scenes, orchestrators, return_payoffs_sum)
...
class Simulation(scenarios_lib.RunnableSimulationWithMemories):
  ...
  def __init__(
    ...
    self._orchestrator_memory = orchestrator_memory
    self._primary_environment = conversation.make_conversation_orchestrator(
