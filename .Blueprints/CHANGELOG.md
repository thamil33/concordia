...
### Changed
- Changed GameMaster to Orchestrator
- Project directory cleanup
- Pyproject reformat

### Added
- LMStudio API Support
- OpenRouter API Support
- Began to implement Documentation
...
- Allow conversation orchestrator to be configured with memory and additional components
...
- Improve logging in orchestrator trigger components.
...
- Improve clarity of orchestrator and fix type annotations
...
- Add Together AI language model to  This can serve Gemma 2 9B
...
- Add check whether orchestrator wants to terminate in the runner.
...
- Make it possible for orchestrator to take a dictionary of action_spec, with a unique action_spec for each player.
...
- Modernize orchestrator concurrency error handling
...
- Make the secondary orchestrators be optional when using the `run_simulation` utility.
...
- Add two useful orchestrator components for creating custom environments.
...
- add a pub coordination simulation - add ability to pass thought chains and maximum conversation length to orchestrator factory
...
- fix typing in orchestrator factory run_simulation.
- Fix bug in orchestrator factory that was causing it to rebuild the memory on every call to build_orchestrator, ignoring the optionally provided memory.
...
- Make components and memory used in orchestrator factory configurable.
- Add wrapper for Mistral language models.
- Add generic launch script to run experiments without a notebook.
- Add configurable factories to create agents and orchestrators. Also add a preliminary version of a modular reality show environment.
...
- Fix a bug that arises with some completion models where the response to a  specific quote processing thought in the orchestrator sometimes ends up never terminating.
...
- Add orchestrator components contrib directory to house components contributed
by users.
- Add orchestrator contrib component: `world_background_and_relevance`.
...
- Add Schelling diagram payoffs computation for orchestrator
...
- Prevent the orchestrator from inventing direct quotes
...
- Encapsulate default orchestrator instructions inside orchestrator class.
...
- Add a current scene component to make the orchestrator aware of the scene
- Adding the recursive component update to the orchestrator, same logic as in the agent.
...
- Improved steps for the orchestrator's thought chain.
...
- Allow orchestrator to account for agency of inactive players.
